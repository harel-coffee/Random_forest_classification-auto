...
def argParser():
	...
	p.add_argument("-i", dest="infile",metavar="input single cell matrix from reference sample", required=True,help="input single cell expression matrix from reference sample")
	p.add_argument("-i2", dest="infile2",metavar="input single cell expression matrix from query sample ", required=True,help="input single cell matrix from query sample")
	p.add_argument("-g", dest="gene",metavar="gene id of 1000 most important features from reference sample", required=True,help="gene id of 1000 most important features from reference sample")
	p.add_argument("-g2", dest="gene2",metavar="gene id of 1000 most important features from query sample", required=True,help="gene id of 1000 most important features from query sample")
	p.add_argument("-c", dest="clusters",metavar="clusters for cells for reference sample", required=True,help="cluster of cells for reference sample")
	if len(sys.argv)==1:
